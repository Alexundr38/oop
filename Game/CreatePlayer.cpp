...
void CreatePlayer::infoManager(int number, std::vector <int> lenghts) {
	...
	oss << "Â èãðå áóäåò " << number << " êîðàáëåé.\n";
	output_manager->printMessage(oss.str());
	for (int i = 0; i < 4; i++) {
		oss.str("");
		oss.clear();
		oss << lenghts[i] << " " << 4 - i << "-ïàëóáíûõ êîðàáëåé;\n";
		output_manager->printMessage(oss.str());
	}
}
...
void CreatePlayer::inputSizes() {
	std::cout << " - Ââåäèòå ðàçìåð ïîëÿ â ôîðìàòå: [øèðèíà âûñîòà]\n";
	srand(time(0));
...
void CreatePlayer::inputAnswer(bool new_iteration) {
	output_manager->printMessage(" - Åñëè õîòèòå ðàññòàâèòü êîðàáëè âðó÷íóþ, ââåäèòå [1]\n - Åñëè õîòèòå, ÷òîáû êîðàáëè áûëè ðàññòàâëåíû àâòîìàòè÷åñêè, ââåäèòå [0]\n");
...
void CreatePlayer::inputCoordinates() {
	...
	while (i < lenghts.size()) {
		std::ostringstream oss;
		oss << " - Ââåäèòå êîîðäèíàòû êîðàáëÿ ðàçìåðà " << lenghts[i] << " è åãî îðèåíòàöèþ íà ïîëå â ôîðìàòå: [y x orientation]\n";
		output_manager->printMessage(oss.str());
		output_manager->printMessage(" - y è x - êîîðäèíàòû ëåâîãî âåðõíåãî óãëà êîðîáëÿ, orientation - åãî îðèåíòàöèÿ íà ïîëå, ïðè ýòîì 1 - ãîðèçîíòàëüíî, 0 - âåðòèêàëüíî.\n");
		int x, y, orient;
...
void CreatePlayer::randomCoordinates() {
	...
	while (i < lenghts.size()) {
		try {
			...
			output_manager->printMessage("Ðàññòàíîâêà êîðàáëåé..\n");
		}
		int orient;
		orient = rand() % 2;
