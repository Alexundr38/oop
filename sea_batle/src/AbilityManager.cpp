...
std::string AbilityManager::abilityToString(Ability* ability) const{
	...
	if (name_ability == "Сканер") return "Scaner";
}
...
void AbilityManager::from_json(const json& j) {
	...
	for (const auto& abilitys : j.at("ability_queue")) {
		if (abilitys == "Bombing") ability_queue.push_back(new Bombing);
		if (abilitys == "DoubleDamage") ability_queue.push_back(new DoubleDamage);
		if (abilitys == "Scaner") ability_queue.push_back(new Scaner);
	}
}
...
void AbilityManager::useAbility(Player* player) {
	bool check = ability_queue[0]->useAbility(player);
