...
std::pair<int, int> GameOperation::inputSize() {
	int first, second;
	std::string input;
	while (true) {
...
std::pair<int, int> GameOperation::inputYX(int height, int width) {
	int y, x;
	std::string input;
	while (true) {
		std::getline(std::cin, input);
...
std::vector<int> GameOperation::inputYXOrient(int height, int width) {
	int y, x, orient;
	std::string input;
	while (true) {
		std::getline(std::cin, input);
...
void GameOperation::inputSizes() {
	...
	std::pair<int, int> sizes = inputSize();
...
void GameOperation::inputCoordinates() {
	...
	while (i < lenghts.size()) {
		...
		int x, y, orient;
		std::pair <int, int> sizes = player_field->getSize();
		std::vector <int> input = inputYXOrient(sizes.first, sizes.second);
		y = input[0];
		x = input[1];
		orient = input[2];
		bool orientation;
...
bool GameOperation::playerMove(bool* flag) {
	if (*flag == true) {
		...
		if (player_answer == 0) {
			...
			ability_manager->useAbility(bot_field, bot_manager);
			bool check_win = bot_manager->checkWin();
			if (check_win == true) {
				*flag = false;
			}
...
void GameOperation::hit(int y, int x, int index, bool flag) {
	if (flag == true) {
		bool check = bot_manager->checkDestroyed(index);
		if (check == true) {
			std::cout << "Попадание в уничтоженный корабль!\n";
		}
		else {
		...
		if (check == true) {
			...
			check = player_manager->checkDestroyed(index);
